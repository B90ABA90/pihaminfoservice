...
import heroEditorial from "@/assets/hero-piham-1.jpg";
import heroSlide2 from "@/assets/hero-piham-2.jpg";
import heroSlide3 from "@/assets/hero-piham-3.jpg";
import heroSlide4 from "@/assets/hero-piham-4.jpg";
import heroSlide5 from "@/assets/hero-piham-5.jpg";
import heroSlide6 from "@/assets/hero-piham-6.jpg";
...
// 3D tilt card driven by mouse position. No-op on mobile/touch.
...
export const Hero = () => {
  ...
  // Preload slides — eagerly load only the first slide; the rest load
  // during browser idle time so the hero paints fast on mobile.
  const [loadedCount, setLoadedCount] = useState(0);
  useEffect(() => {
    let cancelled = false;
    setLoadedCount(0);
    let count = 0;
    const loadOne = (src: string, eager: boolean) => {
      const img = new Image();
      img.decoding = "async";
      if (eager) (img as any).fetchPriority = "high";
      const done = () => {
        if (cancelled) return;
        count += 1;
    ...
    if (heroSlides[0]) loadOne(heroSlides[0], true);
    const idle = (cb: () => void) =>
    ...
    const handle = idle(() => {
      if (cancelled) return;
      heroSlides.slice(1).forEach((s) => loadOne(s, false));
    });
    return () => {
      cancelled = true;
  ...
  return (
    <HeroSection>
      ...
      <div className="container relative py-12 md:py-20">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-8 items-end">
          <div className="lg:col-span-8">
            <h1 className="font-display font-semibold text-foreground text-[10vw] sm:text-[8.5vw] lg:text-[6.4vw] leading-[1.05] tracking-[-0.035em] [hyphens:none] [word-break:normal] [overflow-wrap:normal]">
      ...
      {/* Marquee ticker */}
      <div className="relative border-y border-border bg-background-elevated overflow-hidden py-6 mt-8 shadow-sm">
                ...
                {words.map((w) => (
                  <div key={w.key + k} className="flex items-center gap-12 text-foreground">
                    <button
                      ...
                      className="font-display text-lg sm:text-2xl md:text-3xl tracking-tight font-medium cursor-pointer bg-transparent border-0 p-0 hover:text-[hsl(var(--accent))] transition-colors"
                    >
                      {w.text}
                    </button>
