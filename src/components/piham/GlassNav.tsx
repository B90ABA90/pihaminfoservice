...
export const GlassNav = () => {
  ...
  const goToSection = (hash: string) => {
    const doScroll = () => {
      ...
      if (!el) return;
      // Cherche le conteneur scrollable (le <main> a overflow-x-hidden,
      // ce qui en fait le contexte de scroll sur certains navigateurs).
      const headerH = 80;
      ...
      const scroller =
        ...
        document.body;
      const top = rect.top + scroller.scrollTop - headerH;
      window.scrollTo({ top, behavior: "smooth" });
      history.replaceState(null, "", `#${hash}`);
    };
    setOpen(false);
    if (location.pathname === "/") {
      // Délai pour laisser le menu mobile commencer sa fermeture.
