...
import { Picture } from "@/components/piham/Picture";
    ...
    <main className="relative bg-background text-foreground min-h-screen overflow-x-hidden" data-service-detail={sectionKey ?? undefined}>
      ...
      <div className="container pt-28 md:pt-32 pb-20">
        ...
        <div className="mt-6 grid grid-cols-4 grid-rows-2 gap-2 rounded-2xl overflow-hidden h-[280px] md:h-[480px]">
          ...
          >
            <Picture src={gallery[0].src} alt={service.title} loading="eager" fetchPriority="high" sizes="(min-width: 768px) 50vw, 100vw" data-editable-key={gallery[0].key} className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-105" />
          </button>
          {gallery.slice(1, 5).map((img, i) => (
            ...
            >
              <Picture src={img.src} alt={`${service.title} ${i + 2}`} sizes="25vw" data-editable-key={img.key} className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-105" />
              {i === 3 && (
